...
private:
    ...
    std::vector<std::vector<char>> boardBody;
...
public:
    ...
    // that breaks rules
    bool ValidMove(std::vector<std::vector<char>> boardBody, int x, int y);
    
    // after checking if valid places the queen at provided cords
    //returns true if solutions found other wise false
    bool PlaceQueen(std::vector<std::vector<char>> &board, int col,int queens);
